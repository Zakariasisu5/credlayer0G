...
function fineTuning(program) {
    program
        ...
        (0, util_1.withFineTuningBroker)(options, async (broker) => {
            const result = await broker.fineTuning.verifyService(options.provider, options.outputDir, (step) => console.log(step.message));
            if (!result) {
    ...
    program
        ...
        (0, util_1.withFineTuningBroker)(options, async (broker) => {
            ...
            try {
                ...
                const availableBalance = accountDetail.account.balance -
    ...
    program
        ...
        .option('--inference-provider <address>', 'Inference provider address for --deploy (defaults to --provider if not specified)')
        ...
        (0, util_1.withFineTuningBroker)(options, async (broker) => {
            ...
            if (options.deploy) {
                const inferenceProvider = options.inferenceProvider || options.provider;
                console.log('\nWaiting for inference broker to download the adapter...');
                await deployAdapterToBroker(broker, inferenceProvider, options.model, options.taskId, true, 180);
            }
        });
    });
    program
        .command('acknowledge-deliverable')
        .description('Acknowledge a delivered task on-chain WITHOUT downloading the model. ' +
        ...
        .action((options) => {
        (0, util_1.withFineTuningBroker)(options, async (broker) => {
            await broker.fineTuning.acknowledgeDeliverable(options.provider, options.taskId, options.gasPrice);
            console.log(`Acknowledged deliverable for task ${options.taskId}. ` +
    ...
    program
        ...
        (0, util_1.withBroker)(options, async (broker) => {
            if (options.adapterName) {
                await deployAdapterByName(broker, options.provider, options.adapterName, options.wait, parseInt(options.timeout));
            }
            else {
    ...
    program
        ...
        (0, util_1.withBroker)(options, async (broker) => {
            if (options.taskId && !options.adapterName) {
                adapterName = await broker.inference.resolveAdapterName(options.provider, options.taskId, options.model || '');
            }
            console.log(chalk_1.default.gray(`Adapter model name: ${adapterName}`));
            const resp = await broker.inference.chatWithFineTunedModel(options.provider, adapterName, options.message, { systemPrompt: options.system });
...
async function deployAdapterToBroker(broker, providerAddress, baseModel, taskId, wait, timeoutSeconds) {
    ...
    const result = await broker.inference.deployAdapter(providerAddress, baseModel, taskId, {
        wait,
        timeoutSeconds,
        onProgress: (state) => {
...
async function deployAdapterByName(broker, providerAddress, adapterName, wait, timeoutSeconds) {
    console.log(chalk_1.default.gray(`Adapter name: ${adapterName}`));
    const result = await broker.inference.deployAdapterByName(providerAddress, adapterName, {
        wait,
        timeoutSeconds,
        onProgress: (state) => {
            console.log(chalk_1.default.gray(`  Adapter state: ${state}`));
