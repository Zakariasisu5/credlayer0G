...
class Provider {
     ...
     *   focused.
     */
    async downloadLoRAFromTEE(providerAddress, taskId, outputPath, options) {
        const idleTimeoutMs = options?.idleTimeoutMs ?? 60_000;
        const maxRetries = options?.maxRetries ?? 2;
        try {
            const fs = await Promise.resolve().then(() => tslib_1.__importStar(require('fs/promises')));
            const path = await Promise.resolve().then(() => tslib_1.__importStar(require('path')));
            const { pipeline } = await Promise.resolve().then(() => tslib_1.__importStar(require('stream/promises')));
            const url = await this.getProviderUrl(providerAddress);
            const userAddress = this.contract.getUserAddress();
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                ...
                const attemptLabel = maxRetries > 0
                    ...
                    : '';
                console.log(`Downloading LoRA model from TEE: ${endpoint}${attemptLabel}`);
                ...
                try {
                    ...
                    const { createWriteStream } = await Promise.resolve().then(() => tslib_1.__importStar(require('fs')));
                    ...
                    if (attempt >= maxRetries) {
                        throw new Error(`TEE download failed after ${maxRetries + 1} attempt(s) ` +
                            `(${bytesReceived} bytes received before failure): ` +
                            `${err?.message ?? err}`);
                    }
                    const backoffMs = Math.min(2_000 * 2 ** attempt, 30_000);
                    console.warn(`TEE download attempt ${attempt + 1} failed at ` +
