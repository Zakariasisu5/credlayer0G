...
class ZGServingUserBrokerBase {
    ...
    static MIN_LOCKED_BALANCE = BigInt(1) * BigInt(10 ** 18);
    ledger;
    ...
    static FEE_CACHE_TTL = 24 * 60 * 60 * 1000; // 24 hours
    /**
    ...
    async updateCachedFee(provider, fee) {
        try {
            ...
            await this.cache.setItem(cacheFundKey, BigInt(curFee) + fee, ZGServingUserBrokerBase.FEE_CACHE_TTL, storage_1.CacheValueTypeEnum.BigInt);
        }
        catch (error) {
            (0, utils_1.throwFormattedError)(error);
        }
    ...
    async clearCacheFee(provider) {
        try {
            ...
            await this.cache.setItem(key, BigInt(0), ZGServingUserBrokerBase.FEE_CACHE_TTL, storage_1.CacheValueTypeEnum.BigInt);
        }
        catch (error) {
            (0, utils_1.throwFormattedError)(error);
        }
     ...
     */
    async startAutoFunding(provider, config, gasPrice) {
        ...
        logger_1.logger.debug(`[Auto-funding] Started for provider ${provider} ` +
     ...
     */
    stopAutoFunding(provider) {
        if (provider) {
            const timer = this.autoFundingTimers.get(provider);
            if (timer) {
                clearInterval(timer);
                this.autoFundingTimers.delete(provider);
                logger_1.logger.debug(`[Auto-funding] Stopped for provider ${provider}`);
            }
        }
        else {
            // Stop all
            for (const [addr, timer] of this.autoFundingTimers) {
                clearInterval(timer);
                logger_1.logger.debug(`[Auto-funding] Stopped for provider ${addr}`);
            }
     ...
     */
    async checkAndFund(provider, bufferMultiplier, gasPrice) {
        try {
            ...
            const requiredBalance = unsettledFee + BigInt(bufferMultiplier) * minLocked;
            logger_1.logger.debug(`[Auto-funding] Check: unsettledFee=${this.neuronToA0gi(unsettledFee).toFixed(6)} 0G, ` +
                `requiredBalance=${this.neuronToA0gi(requiredBalance).toFixed(6)} 0G`);
            const deficit = await this.getTransferDeficit(provider, requiredBalance);
            if (deficit > BigInt(0)) {
                await this.doTransfer(provider, deficit, gasPrice);
            }
        }
        catch (error) {
            logger_1.logger.warn(`[Auto-funding] Check-and-fund failed: ${error?.message || error}`);
        }
    }
    /**
     ...
     */
    async fetchUnsettledFee(provider) {
        try {
            const svc = await this.getService(provider);
            const headers = await this.getHeader(provider);
            const userAddress = this.contract.getUserAddress();
            const response = await fetch(`${svc.url}/v1/user/${userAddress}/unsettledfee`, {
                method: 'GET',
                headers: {
                    ...headers,
                },
            ...
            if (!response.ok) {
                logger_1.logger.debug(`[Auto-funding] Provider does not support unsettled fee query (${response.status}), using cachedFee fallback`);
                return await this.getCachedFee(provider);
            }
            const data = await response.json();
            const fee = BigInt(data.unsettledFee || '0');
            logger_1.logger.debug(`[Auto-funding] Provider unsettled fee: ${this.neuronToA0gi(fee).toFixed(6)} 0G`);
            // Snapshot the real unsettled fee into cachedFee as fallback
            const cacheFundKey = storage_1.CacheKeyHelpers.getCachedFeeKey(provider);
            await this.cache.setItem(cacheFundKey, fee, ZGServingUserBrokerBase.FEE_CACHE_TTL, storage_1.CacheValueTypeEnum.BigInt);
            return fee;
        }
        catch (error) {
            logger_1.logger.debug(`[Auto-funding] Failed to fetch unsettled fee: ${error?.message || error}, using cachedFee fallback`);
            return await this.getCachedFee(provider);
        }
    }
    async getCachedFee(provider) {
        try {
            const key = storage_1.CacheKeyHelpers.getCachedFeeKey(provider);
     ...
     */
    async getTransferDeficit(provider, requiredBalance) {
        try {
            const acc = await this.contract.getAccount(provider);
            const lockedFund = acc.balance - acc.pendingRefund;
            logger_1.logger.debug(`Locked fund for provider ${provider}: ${lockedFund.toString()}, required: ${requiredBalance.toString()}`);
    ...
    async doTransfer(provider, amount, gasPrice) {
        ...
        try {
            await this.ledger.transferFund(provider, 'inference', transferAmount, gasPrice);
            ...
            if (errorMessage.includes('insufficient')) {
                logger_1.logger.warn(`[Auto-funding] Requires ${amountInOG.toFixed(4)} 0G in your ledger to transfer to the ` +
            ...
            logger_1.logger.warn(`[Auto-funding] Failed to transfer ${amountInOG.toFixed(4)} 0G to provider sub-account: ${error?.message || error}`);
        }
    }
}
